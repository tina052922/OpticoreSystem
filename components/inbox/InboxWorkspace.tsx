...
import type { PortalId } from "@/lib/inbox-store";
import type { WorkflowInboxMessage } from "@/lib/api/client";
...
export function InboxWorkspace({
  ...
  const [mail, setMail] = useState<WorkflowInboxMessage[]>([]);
  const [sent, setSent] = useState<WorkflowInboxMessage[]>([]);
  ...
  async function refresh() {
    try {
      const data = await inboxApi.listPortal(portal);
      setMail(data.mail);
      setSent(data.sent);
      setSelectedId((cur) => {
        const pool = tab === "mail" ? data.mail : data.sent;
  ...
  async function forwardDraftToCollege() {
    ...
    try {
      ...
      alert(e instanceof ApiClientError ? e.message : "Forward failed");
      return;
    }
    await refresh();
    alert(
