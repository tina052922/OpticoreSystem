...
import { policyReviewsApi, ApiClientError } from "@/lib/api/client";
...
function ReviewCard({
  ...
  async function submit(decision: "accepted" | "rejected") {
    ...
    setMsg(null);
    try {
      const data = await policyReviewsApi.review({
        justificationId: row.id,
        decision,
        note: note.trim() || null,
      });
      if (data.justification) {
        onDecisionSaved(row.id, data.justification as ScheduleLoadJustification);
      }
      if (data.warning) {
        setMsg(data.warning);
      }
