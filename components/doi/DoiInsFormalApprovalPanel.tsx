...
export function DoiInsFormalApprovalPanel({
  ...
  const loadFinalization = useCallback(async () => {
    ...
    if (!id) return;
    try {
      const data = await doiApi.getFinalization(id);
  ...
  async function runCampusConflictCheck() {
    ...
    setConflict(null);
    try {
      const data = await doiApi.getScheduleConflicts(periodId);
      ...
      setConflictError(e instanceof ApiClientError ? e.message : e instanceof Error ? e.message : "Check failed");
    } finally {
  ...
  async function submitDecision(action: "approve" | "reject" | "unpublish") {
    ...
    try {
      const data = await doiApi.patchFinalization({
      ...
      setDecisionError(e instanceof ApiClientError ? e.message : e instanceof Error ? e.message : "Update failed");
    } finally {
