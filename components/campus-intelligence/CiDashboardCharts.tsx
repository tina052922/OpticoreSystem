...
export function CiDashboardCharts({
  ...
  // Use server-fetched data if provided; otherwise fetch client-side
  useEffect(() => {
    if (hasServerData) {
      setRoomUtilizationData(chartsData.roomUtilizationBySlot);
      setFacultyLoadData(chartsData.facultyLoadDistribution);
      setLoading(false);
      setError(null);
      return;
    }
    ...
    void (async () => {
      try {
        ...
        if (!res.ok) {
          if (!cancelled)
            setError(j?.error ?? "Could not load dashboard analytics.");
          return;
        }
        if (!cancelled) {
  ...
  }, [analyticsScope, chartsData, hasServerData]);

  if (!analyticsScope) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="h-80 rounded-lg bg-gray-100/80 border border-gray-200" />
        <div className="h-80 rounded-lg bg-gray-100/80 border border-gray-200" />
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="h-80 rounded-lg bg-gray-100/80 animate-pulse border border-gray-200" />
        <div className="h-80 rounded-lg bg-gray-100/80 animate-pulse border border-gray-200" />
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
        ...
        <p className="text-xs text-gray-600 mb-3">
          Instructor counts by weekly contact-band.
        </p>
        {hasFacultyData ? (
          <div className="flex flex-col sm:flex-row items-center justify-center min-h-62.5 gap-4">
