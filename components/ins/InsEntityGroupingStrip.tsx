...
import { isStudentInsPortal, showInsFacultyGrouping } from "@/lib/ins/schedule-visibility";
...
export function InsEntityGroupingStrip({ insBasePath, facultyCount, sectionCount, roomCount }: Props) {
  const enabled =
    ...
    isStudentInsPortal(insBasePath);
  if (!enabled) return null;

  const facultyGrouping = showInsFacultyGrouping(insBasePath);
