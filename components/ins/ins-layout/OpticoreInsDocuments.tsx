              ...
              {(items as InsFacultyCell[])
                ...
                .map((classAtTime, idx) => {
                  ...
                  return (
                    <div
                      key={
                        classAtTime.scheduleEntryId ??
                        `${classAtTime.time}-${idx}`
                      }
                    >
  ...
  return (
    <div className="space-y-4">
      ...
      {slot?.signerName && slot.signerName !== "—" ? (
        ...
        <div className="text-center text-[11px] text-amber-900">
          No signature on file — upload in Profile
        </div>
      ) : (
              ...
              {(items as SectionScheduleCell[]).slice(0, 3).map((row, idx) => {
                const inner = (
                  <div className="w-full space-y-0.5 print:space-y-0 text-xs print:text-[6.5pt] leading-snug break-words">
                    ...
                    <div className="text-[10px] text-neutral-600 print:text-[6pt]">
                      {row.time}
                    </div>
