...
export type BsitChairmanInteractiveWeekGridProps = {
  ...
  /** Academic period ID for conflict checking in the plot modal. */
...
export function BsitChairmanInteractiveWeekGrid({
  ...
  const existingBlocksForModal = useMemo(
    () => (modal ? sparseCampusUniverse.filter((b) => b.id !== modal.draft.id) : []),
    [sparseCampusUniverse, modal],
  );
