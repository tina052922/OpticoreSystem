...
export function EvaluatorTimetablingPanel({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadError("Could not load evaluator data via API.");
      setLoading(false);
    }
  }, [academicPeriodId]);
    ...
    void (async () => {
      try {
        const data = await apiFetch<{ justifications: ScheduleLoadJustification[] }>(
          `/api/catalog/schedule-load-justifications?academicPeriodId=${academicPeriodId}&collegeId=${effectiveCollegeId}`,
          { method: "GET" },
        );
        if (!cancelled) {
  ...
  const instructorLoadById = useMemo(() => {
    ...
    for (const r of policyEvaluation.rows) {
    ...
    return m;
  }, [policyEvaluation.rows, profileByUserId, policyConstants]);
  ...
  /** Conflict scan for College Admin / CAS / GEC / DOI is campus-wide for the term (filters are browsing only). */
  ...
  }, [mergedScheduleEntries, academicPeriodId]);
  ...
  function runFullConflictCheck() {
    ...
    void (async () => {
      ...
      try {
        let j: CampusConflictScanResult & { enrichedIssues?: typeof localEnriched };
        try {
          j = (await schedulingApi.scopeConflictScan({
            academicPeriodId,
            mode: "doi_campus",
            collegeId: null,
            programId: null,
          })) as CampusConflictScanResult & { enrichedIssues?: typeof localEnriched };
        } catch (e) {
          const { conflictSet, summaryLines } = applyLocalMerge([], [], []);
          const msg = e instanceof ApiClientError ? e.message : "Please try again.";
          if (conflictSet.size === 0) {
            toast.error("Conflict scan failed", msg);
          } else {
            toast.error(
              "Server scan failed",
              `${summaryLines.length} issue(s) from plotted rows (including drafts). ${msg}`.trim(),
            );
          }
          return;
        }
  ...
  const selectedInstructorLoad = instructorId ? (instructorLoadById.get(instructorId) ?? null) : null;
  const projectedInstructorHours = selectedInstructorLoad
    ? selectedInstructorLoad.hours + subjectContactHours
    : null;
  ...
  async function saveToDatabase(opts?: { skipJustificationPrompt?: boolean }) {
    ...
    try {
          ...
          await apiFetch("/api/catalog/schedule-load-justifications", {
            ...
            body: {
              ...
              violationsSnapshot: {
                ...
                scheduleEntryIds: rows.filter((r) => r.instructorId === v.instructorId).map((r) => r.id),
      ...
      await apiFetch("/api/catalog/schedule-entries-upsert", {
        method: "POST",
        body: rows,
      });
      ...
      if (author?.role === "chairman_admin" && effectiveCollegeId && rows.length > 0) {
        try {
          const admins = await apiFetch<{ users: { id: string }[] }>(
            `/api/catalog/users?role=college_admin&collegeId=${effectiveCollegeId}`,
            { method: "GET" },
          );
          if (admins.users?.length) {
            ...
            for (const a of admins.users) {
              await apiFetch("/api/notifications", {
                method: "POST",
                body: { userId: a.id, message: msg },
              });
            }
          }
        } catch {}
