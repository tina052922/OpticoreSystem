...
export function CentralHubEvaluatorView({
  ...
  const collegeSlug = isHubCollegeListView(searchParams.get("view"), searchParams.get("college"))
    ? null
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        setMyCollegeId(user?.collegeId?.trim() || null);
        setMyUserId(user?.id?.trim() || null);
      } catch {
        if (cancelled) return;
      }
  ...
  const load = useCallback(async (opts?: { soft?: boolean }) => {
    ...
    try {
      const pid = academicPeriodId || "";
      const bundle = await apiFetch<any>(
        `/api/catalog/evaluator-bundle?academicPeriodId=${pid}`,
        { method: "GET" },
      );
      if (!soft) {
  ...
  const runScopedConflictScan = useCallback(() => {
    ...
    void (async () => {
      ...
      try {
        let j: CampusConflictScanApiPayload;
        try {
          ...
          if (payload.conflictingEntryIds.length === 0) {
            ...
            toast.error(
              "Server scan failed",
              `${payload.issueSummaries.length} issue(s) from hub timetable. ${msg}`.trim(),
            );
          }
          return;
        }
          ...
          {crossPending ? (
            <div className="rounded-xl border border-amber-200 bg-amber-50/90 p-6 text-[14px]">
              ...
              <p className="mt-2 text-black/80">
                ...
                you can open this college in <strong>view-only</strong> mode (no edits).
              </p>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-2 text-[14px] text-black/75">
                ...
                After approval you may <strong>view only</strong> — you cannot edit their timetable.
              </p>
                ...
                onClick={async () => {
                  ...
                  setAccessRequestBusy(true);
                    try {
                      ...
                      setAccessRequestErr(e instanceof ApiClientError ? e.message : "Request failed");
                    } finally {
        ...
        {panel === "timetabling" ? (
          <>
            ...
            <DoiScheduleEntryQuickEditDialog
                open={Boolean(editEntryId) && !hubReadOnly}
                onOpenChange={(o) => {
                ...
                onSave={async (patch) => {
                  ...
                  setEditBusy(true);
                    try {
                    await apiFetch(`/api/catalog/schedule-entries/${savedId}`, {
                      method: "PATCH",
                      body: patch,
                    });
                    setEditEntryId(null);
