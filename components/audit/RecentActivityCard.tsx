...
import { formatAuditDetailsEnglish } from "@/lib/audit/format-audit-entry";
...
export function RecentActivityCard({ auditHref = "/admin/college/audit-log" }: Props) {
  ...
  const load = useCallback(async () => {
    try {
      const data = await auditLogApi.list({ limit: 6 });
      setEntries(data.entries as AuditEntry[]);
    } catch {
      /* ignore transient errors on dashboard widget */
    }
  }, []);
        ...
        <ul className="space-y-2 text-sm">
          {entries.map((e) => (
            <li key={e.id} className="flex flex-col gap-0.5 border-b border-black/5 pb-2 last:border-0">
              ...
              <span className="font-medium text-black/85 leading-snug">{formatAuditDetailsEnglish(e)}</span>
