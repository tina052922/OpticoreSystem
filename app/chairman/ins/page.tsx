  ...
  return (
    <Button
      ...
      className={cn(
        "h-9 px-3 text-sm",
        active
          ? "bg-[#FF990A] hover:bg-[#FF990A]/90 text-white border-transparent"
          : "bg-white",
      )}
    >
...
export default async function ChairmanInsIndexPage({
  ...
  const activeTab: TabKey =
    requestedTab === "section" || requestedTab === "room"
      ? requestedTab
      : "faculty";
  const initialSectionId =
    typeof sp.sectionId === "string" ? sp.sectionId : null;
      ...
      <div className="px-4 sm:px-6 lg:px-8 pb-6 space-y-4">
        <div className="flex flex-wrap gap-2">
          ...
          <TabLink
            tab="section"
            activeTab={activeTab}
            href="/chairman/ins?tab=section"
            label="Section view"
          />
          <TabLink
            tab="room"
            activeTab={activeTab}
            href="/chairman/ins?tab=room"
            label="Room view"
          />
        </div>
